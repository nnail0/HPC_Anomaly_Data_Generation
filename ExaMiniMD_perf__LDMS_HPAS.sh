...
echo "ExaMiniMD"
srun --exclusive --ntasks=1  --cpus-per-task=1 --mem=1G ldmsd -x sock:20001 -l logs/aggregator.log -c conf/aggregator.conf &
LDMS_AGG_PID=$!
APP_START_TIME=$(date +%s)
srun --exclusive --ntasks=1 --cpu-bind=verbose  --cpus-per-task=8 --mem=20G ExaMiniMD -il ./input_files/ExaMiniMD_in.lj --comm-type MPI
APP_END_TIME=$(date +%s)
...
srun --exclusive  --cpus-per-task=1 --mem=1G ldmsd -x sock:20001 -l logs/aggregator.log -c conf/aggregator.conf &
LDMS_AGG_PID=$!
srun --exclusive --cpu-bind=verbose  --cpus-per-task=1 --mem=20G hpas cpuoccupy -u 95, -t $ANOM_START_TIME -d $(($ANOM_END_TIME - $ANOM_START_TIME)) &
ANOM_PID=$!
srun --exclusive --cpu-bind=verbose  --cpus-per-task=8 --mem=20G ExaMiniMD -il ./input_files/ExaMiniMD_in.lj --comm-type MPI
kill $LDMS_AGG_PID
kill $LDMS_SAMPLER_PID
...
srun --exclusive  --cpus-per-task=1 --mem=1G ldmsd -x sock:20001 -l logs/aggregator.log -c conf/aggregator.conf &
LDMS_AGG_PID=$!
srun --exclusive --cpu-bind=verbose  --cpus-per-task=1 --mem=20G hpas memleak -s 10M, -t $ANOM_START_TIME -d $(($ANOM_END_TIME - $ANOM_START_TIME)) &
ANOM_PID=$!
srun --exclusive --cpu-bind=verbose  --cpus-per-task=8 --mem=20G ExaMiniMD -il ./input_files/ExaMiniMD_in.lj --comm-type MPI
kill $LDMS_AGG_PID
kill $LDMS_SAMPLER_PID
...
srun --exclusive  --cpus-per-task=1 --mem=1G ldmsd -x sock:20001 -l logs/aggregator.log -c conf/aggregator.conf &
LDMS_AGG_PID=$!
srun --exclusive --cpu-bind=verbose  --cpus-per-task=1 --mem=20G hpas cachecopy -c L1 -m 0.8, -t $ANOM_START_TIME -d $(($ANOM_END_TIME - $ANOM_START_TIME)) &
ANOM_PID=$!
srun --exclusive --cpu-bind=verbose  --cpus-per-task=8 --mem=20G ExaMiniMD -il ./input_files/ExaMiniMD_in.lj --comm-type MPI
kill $LDMS_AGG_PID
kill $LDMS_SAMPLER_PID
...
srun --exclusive  --cpus-per-task=1 --mem=1G ldmsd -x sock:20001 -l logs/aggregator.log -c conf/aggregator.conf &
LDMS_AGG_PID=$!
srun --exclusive --cpu-bind=verbose  --cpus-per-task=1 --mem=20G hpas cachecopy -c L2 -m 0.8, -t $ANOM_START_TIME -d $(($ANOM_END_TIME - $ANOM_START_TIME)) &
ANOM_PID=$!
srun --exclusive --cpu-bind=verbose  --cpus-per-task=8 --mem=20G ExaMiniMD -il ./input_files/ExaMiniMD_in.lj --comm-type MPI
kill $LDMS_AGG_PID
kill $LDMS_SAMPLER_PID
...
srun --exclusive  --cpus-per-task=1 --mem=1G ldmsd -x sock:20001 -l logs/aggregator.log -c conf/aggregator.conf &
LDMS_AGG_PID=$!
srun --exclusive --cpu-bind=verbose  --cpus-per-task=1 --mem=20G hpas cachecopy -c L3 -m 0.8 -t $ANOM_START_TIME -d $(($ANOM_END_TIME - $ANOM_START_TIME)) &
ANOM_PID=$!
srun --exclusive --cpu-bind=verbose  --cpus-per-task=8 --mem=20G ExaMiniMD -il ./input_files/ExaMiniMD_in.lj --comm-type MPI
kill $LDMS_AGG_PID
kill $LDMS_SAMPLER_PID
